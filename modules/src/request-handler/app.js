...
const shortUid = () => getRandomInt(1000);
...
const handleRequest = async (event) => {
    ...
    if (event.path === '/request-handler' && event.httpMethod === 'POST') {
        console.log("POST")
        return startNewRequest(event);
    } else if (event.path === '/request-handler' && event.httpMethod === 'GET') {
        console.log("GET")
...
const startNewRequest = async () => {
    ...
    const requestID = shortUid().toString();
    console.log("type of...")
